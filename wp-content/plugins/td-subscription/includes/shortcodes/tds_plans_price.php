        ...
        $buffy = '<div class="' . $block_classes . '" ' . $this->get_block_html_atts() . ' ' . $def_type_from_block_data . '>';
            ...
            $buffy .= '<div class="tds-block-inner td-fix-index">';
			    ...
			    if ( !empty($free_plan) ) {
                    ...
                    if ( array_key_exists($free_plan, $free_plans) ) {
                        $buffy .= '<div class="td-pp-price-txt">' . __td('Free', TD_THEME_NAME) . '</div>';
                    } else {

                        if( tdc_state::is_live_editor_ajax() || tdc_state::is_live_editor_iframe() || is_user_logged_in() ) {
                            $buffy .= '<div class="td-pp-price-txt" title="Selected free plan was not found, please set a valid free plan">n/a</div>';
                        }
					    ...
					    if ( count($all_plans) ) {
						    ...
						    $buffy .= '<div class="tds-plans-price tds-show-plan-price ' . $default_plan_class . '" data-type="year">';
                                ...
                                if ( !empty($all_plans[$plan1]) ) {
                                    ...
                                    if( tdc_state::is_live_editor_ajax() || tdc_state::is_live_editor_iframe() || is_user_logged_in() ) {
                                        $buffy .= '<div class="td-pp-price-txt" title="Selected plan 1 was not found, please set a valid plan 1">n/a</div>';
                                    }
					    ...
					    if ( count($all_plans) ) {
	                        ...
	                        $buffy .= '<div class="tds-plans-price tds-show-plan-price ' . $default_plan_class . '" data-type="month">';
                                ...
                                if ( !empty($all_plans[$plan2]) ) {
                                    ...
                                    if( tdc_state::is_live_editor_ajax() || tdc_state::is_live_editor_iframe() || is_user_logged_in() ) {
                                        $buffy .= '<div class="td-pp-price-txt" title="Selected plan 2 was not found, please set a valid plan 2">n/a</div>';
                                    }
