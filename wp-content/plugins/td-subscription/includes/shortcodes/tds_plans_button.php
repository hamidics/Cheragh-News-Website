...
class tds_plans_button extends td_block {
	...
	function render( $atts, $content = null ) {
		...
		$buffy = '<div class="' . $block_classes . '" ' . $this->get_block_html_atts() . '>';
            ...
            $plan_1 = $this->get_att( 'year_plan' );
            $plan_2 = $this->get_att( 'month_plan' );
            ...
            $buffy .= '<div class="tds-block-inner td-fix-index">';
				...
				} else if ( !empty($free_plan) && !array_key_exists($free_plan, $free_plans) ) {
                    $buffy .= td_util::get_block_error( 'Plans button', 'Please set a valid "Free plan"' );
				} else if ( empty($free_plan) && !empty($plan_1) && !array_key_exists($plan_1, $all_plans) ) {
                    $buffy .= td_util::get_block_error( 'Plans button', 'Please set a valid "Plan 1"' );
				} else if ( empty($free_plan) && !empty($plan_2) && !array_key_exists($plan_2, $all_plans) ) {
                    $buffy .= td_util::get_block_error( 'Plans button', 'Please set a valid "Plan 2"' );
				} else {
					$data_url_free = '';
